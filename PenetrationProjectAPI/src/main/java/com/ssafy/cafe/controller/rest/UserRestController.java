...
public class UserRestController {
	...
	@GetMapping("/stamps")
	@ApiOperation(value = "id 사용자의 스탬프 개수를 가져온다.", response = User.class)
	public User selectStamp(String id) {
		return uService.selectStamp(id);
	}
	
	@GetMapping("/age")
	@ApiOperation(value = "id 사용자의 나이를 가져온다.", response = User.class)
	public User selectAge(String id) {
		return uService.selectAge(id);
	}
	
	@GetMapping("/gender")
	@ApiOperation(value = "id 사용자의 성별을 가져온다.", response = User.class)
	public User selectGender(String id) {
		return uService.selectGender(id);
	}
	
	@PutMapping
	@Transactional
	@ApiOperation(value = "user의 stamps를 수정한다.", response = Integer.class)
	public Integer update(@RequestBody User user) {
		return uService.updateStamp(user);
	}

	@PostMapping("/login")
	@ApiOperation(value = "로그인 처리 후 성공적으로 로그인 되었다면 loginId라는 쿠키를 내려보낸다.", response = User.class)
	public User login(@RequestBody User user) {//throws UnsupportedEncodingException { @RequestBody User user, HttpServletResponse response

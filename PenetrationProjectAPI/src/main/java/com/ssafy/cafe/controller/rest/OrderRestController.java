...
public class OrderRestController {
	...
	public List<Map<String, Object>> getOrderDetail(@PathVariable Integer orderId) {
	...
	@GetMapping
	@ApiOperation(value = "모든 주문 정보.", response = List.class)
	public List<Order> selectAll() {
		return oService.selectAll();
	}
	
	@GetMapping("/orderdetail/{orderId}")
	@ApiOperation(value = "{orderId}에 따른 오더 디테일을 가져온다.", response = List.class)
	public List<OrderDetail> select(@PathVariable int orderId) {
		return oService.select(orderId);
	}
	
	@PostMapping("/orderdetail")
	@Transactional
	@ApiOperation(value = "orderdetail 객체를 추가한다.", response = Boolean.class)
	public Boolean insert(@RequestBody OrderDetail orderDetail) {
